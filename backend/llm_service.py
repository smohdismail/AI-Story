...
async def group_chat_with_characters(story_summary: str, characters_info: str, chat_history: str, relevant_memories: str = "") -> str:
    memory_section = f"\n\nRELEVANT PAST MEMORIES:\n{relevant_memories}" if relevant_memories else ""
    messages = [
        {"role": "system", "content": f"You are acting as multiple characters in a group chat with the user in a fictional story.\n\nSTORY CONTEXT:\n{story_summary}\n\nCHARACTERS IN CHAT:\n{characters_info}{memory_section}\n\nYou must generate the NEXT response in the group chat. Pick ONE or TWO characters to reply to the latest message. Do not break character. Respond naturally. Format each character's message starting with 'Name: message'. You must heavily emphasize direct spoken dialogue between the characters and the user. Put any physical actions inside *asterisks* so they are formatted in italics."}
    ]
